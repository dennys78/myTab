...
import { Upload, FileImage, Save, AlertCircle, Loader2, X, Plus, Sparkles, Calculator } from 'lucide-react';
...
export default function AcquisisciChiusureAI({ onBack }) {
  ...
  const handleExtract = () => {
    ...
    fetch('/api/closures/extract-ai/', { method: 'POST', body: formData })
      .then(res => res.json())
      .then(data => {
        if (data.status === 'success') {
          ...
          if (!dataWithIds.date) dataWithIds.date = new Date().toISOString().split('T')[0];
          setPreviewData(dataWithIds);
        } else {
          setError(data.error || "Errore sconosciuto durante l'estrazione.");
        }
      })
      .catch(() => setError('Errore di rete durante la comunicazione con il server.'))
      .finally(() => setLoading(false));
  };

  const handleDateChange = (e) => {
    setPreviewData(prev => ({ ...prev, date: e.target.value }));
  };
  ...
  const handleSummaryChange = (e) => {
    ...
    setPreviewData(prev => ({
      ...prev,
      summary: { ...prev.summary, [name]: parseFloat(value) || 0 },
    }));
  };
  ...
  const handleAddItem = () => {
    setPreviewData(prev => ({
      ...prev,
      items: [...prev.items, { id: `temp_new_${Date.now()}`, descrizione: '', entrate: 0, uscite: 0, saldo: 0 }],
    }));
  };

  const handleRemoveItem = (id) => {
    setPreviewData(prev => ({ ...prev, items: prev.items.filter(item => item.id !== id) }));
  };
  ...
  const handleAcquire = () => {
    ...
    fetch('/api/closures/insert/', {
      ...
      .catch(() => setError('Errore di rete durante il salvataggio.'))
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', padding: '1.5rem', marginBottom: '2rem' }}>
          <h2 style={{ marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <Calculator size={20} color="var(--accent)" /> Dati Generali
          </h2>
          <div style={{ marginBottom: '1.5rem' }}>
            <label style={{ display: 'block', fontSize: '0.875rem', color: 'var(--text-muted)', marginBottom: '0.5rem' }}>Data Chiusura</label>
            <input type="date" value={previewData.date} onChange={handleDateChange} style={{ ...inputStyle, width: '200px' }} />
          </div>
        ...
        <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', padding: '1.5rem', marginBottom: '2rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            ...
            <button onClick={handleAddItem} style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', padding: '0.5rem 1rem', background: 'rgba(59,130,246,0.1)', color: 'var(--accent)', border: '1px solid var(--accent)', borderRadius: '6px', cursor: 'pointer' }}>
                  ...
                  <tr key={item.id}>
                    <td>
                      <input type="text" value={item.descrizione}
                        ...
                        style={{ ...inputStyle, width: '100%', minWidth: '150px' }}
                      />
                    </td>
                    <td>
                    ...
                    <td>
                      <span style={{
                        display: 'inline-block', width: '100px', padding: '0.5rem',
                        fontWeight: '600',
                ...
                {previewData.items.length === 0 && (
                  <tr>
                    <td colSpan="5" style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-muted)' }}>
                      Nessun reparto estratto. Clicca su "Aggiungi Riga".
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '1rem' }}>
          <button onClick={onBack} style={{ padding: '0.75rem 1.5rem', background: 'transparent', border: '1px solid var(--border)', color: 'white', borderRadius: '8px', cursor: 'pointer' }}>
            Annulla
          </button>
  ...
  return (
    <div style={{ maxWidth: '600px', margin: '0 auto', textAlign: 'center' }}>
      <h1 style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
        <Sparkles size={28} color="var(--accent)" /> Acquisisci con IA
      </h1>
      <p style={{ color: 'var(--text-muted)', marginBottom: '0.5rem' }}>
        Carica fino a 2 immagini — Claude Haiku analizza la chiusura e restituisce i dati strutturati.
      </p>
      <p style={{ color: 'var(--text-muted)', fontSize: '0.8rem', marginBottom: '2rem' }}>
        Qualità di lettura superiore rispetto all'OCR classico, anche con foto sfocate o storte.
      </p>
      ...
      <div style={{ border: '2px dashed var(--border)', borderRadius: '12px', padding: '3rem 2rem', background: 'rgba(255,255,255,0.02)', marginBottom: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
        ...
        <input type="file" id="ai-file-upload" multiple accept="image/*" onChange={handleFileChange} style={{ display: 'none' }} />
        <label htmlFor="ai-file-upload" style={{ padding: '0.75rem 1.5rem', background: 'var(--bg-card)', border: '1px solid var(--border)', color: 'white', borderRadius: '8px', cursor: 'pointer', marginTop: '1rem' }}>
          Sfoglia File
        </label>
      </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            {files.map((file, index) => (
              <div key={index} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--bg-card)', padding: '0.75rem 1rem', borderRadius: '8px', border: '1px solid var(--border)' }}>
                ...
                <button onClick={() => handleRemoveFile(index)} style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', cursor: 'pointer' }}>
                  <X size={18} />
                </button>
              </div>
            ))}
      ...
      <button onClick={handleExtract} disabled={files.length === 0 || loading}
        style={{ width: '100%', padding: '1rem', background: files.length > 0 ? 'var(--accent)' : 'var(--bg-card)', color: files.length > 0 ? 'white' : 'var(--text-muted)', border: 'none', borderRadius: '8px', fontSize: '1rem', fontWeight: 'bold', cursor: files.length > 0 && !loading ? 'pointer' : 'not-allowed', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.5rem' }}>
        {loading && <Loader2 size={20} className="spin" />}
        {loading ? 'Analisi IA in corso...' : 'Analizza con Claude IA'}
      </button>
    </div>
  );
}
