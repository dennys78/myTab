                        ...
                        <td colSpan="7" style={{ padding: 0, borderBottom: 'none' }}>
                          <div className="expanded-content">
                            ...
                            <div className="summary-section" style={{ marginBottom: '2rem', padding: '1rem', background: 'var(--bg-dark)', borderRadius: '8px', border: '1px solid var(--border)' }}>
                              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                                ...
                                {editingId === closure.id ? (
                                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                                    <button onClick={() => handleSaveEdit(closure.id)} disabled={saving} className="btn-save" style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', padding: '0.25rem 0.75rem', background: 'var(--success)', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>
                                      <Save size={14} /> {saving ? 'Salvataggio...' : 'Salva'}
                                    </button>
                                    <button onClick={handleCancelEdit} disabled={saving} className="btn-cancel" style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', padding: '0.25rem 0.75rem', background: 'transparent', color: 'var(--text-muted)', border: '1px solid var(--border)', borderRadius: '4px', cursor: 'pointer' }}>
                                      <X size={14} /> Annulla
                                    </button>
                                  </div>
                                ) : (
                              ...
                              <div className="summary-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '1rem' }}>
                                  ...
                                  <div key={key} className="summary-field">
                                    ...
                                    {editingId === closure.id ? (
                                      <input 
                                        type="number" 
                                        name={key}
                                        value={editFormData[key] === 0 && editingId === closure.id ? '' : editFormData[key]} 
                            ...
                            <table className="inner-table">
                              ...
                              <tbody>
                                {closure.items.length > 0 ? closure.items.map(item => {
                                  ...
                                  <tr key={item.id} style={{ cursor: 'default' }}>
                                    ...
                                    <td>
                                      {editingId === closure.id ? (
                                        <input 
                                          type="number"
                                          value={editItem?.entrate === 0 && editingId === closure.id ? '' : editItem?.entrate}
                                          onChange={(e) => handleItemInputChange(item.id, 'entrate', e.target.value)}
                                          style={{ width: '80px', padding: '0.25rem 0.5rem', background: 'var(--bg-card)', border: '1px solid var(--accent)', color: 'var(--text-main)', borderRadius: '4px' }}
                                          placeholder={item.entrate}
                                        />
                                      ) : (
                                    ...
                                    <td>
                                      {editingId === closure.id ? (
                                        <input 
                                          type="number"
                                          value={editItem?.uscite === 0 && editingId === closure.id ? '' : editItem?.uscite}
                                          onChange={(e) => handleItemInputChange(item.id, 'uscite', e.target.value)}
                                          style={{ width: '80px', padding: '0.25rem 0.5rem', background: 'var(--bg-card)', border: '1px solid var(--accent)', color: 'var(--text-main)', borderRadius: '4px' }}
                                          placeholder={item.uscite}
                                        />
                                      ) : (
                                    ...
                                    <td>
                                      {editingId === closure.id ? (
                                        <input 
                                          type="number"
                                          value={editItem?.saldo === 0 && editingId === closure.id ? '' : editItem?.saldo}
                                          onChange={(e) => handleItemInputChange(item.id, 'saldo', e.target.value)}
                                          style={{ width: '80px', padding: '0.25rem 0.5rem', background: 'var(--bg-card)', border: '1px solid var(--accent)', color: 'var(--text-main)', borderRadius: '4px' }}
                                  ...
                                  <tr>
                                    <td colSpan="4" style={{ textAlign: 'center' }}>Nessuna voce trovata</td>
                                  </tr>
                                )}
                              </tbody>
                            </table>
